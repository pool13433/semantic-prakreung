    ...
    public function actionNewsSave() {
        ...
        if ($news->save(false)) {
            ...
            if (!empty($_FILES['image'])) {
                $images = $utility->readArrayFiles($_FILES['image']);
                foreach ($images as $index => $file) {
                    $newsImg = new SacredNewsImg();
                    $newsImg->img_ext = $file['type'];
                    $newsImg->img_size = $file['size'];

                    $newsImg->news_id = $news->news_id;

                    $subDerectoryOther = '/upload_news/' . $currentDate . '_';
                    $imageName = $utility->resizeImagePercent($pathImage . $subDerectoryOther, $file, $this->resizeUpload);

                    $newsImg->img_name = $subDerectoryOther . $imageName;
                    if (!$newsImg->save(false)) {
            ...
            echo CJSON::encode(array(
                ...
                'url' => Yii::app()->createUrl('sacred/indexNews')
            ));
        } else {
            echo CJSON::encode(array(
                'status' => false,
